...
from model.build_graph import PrototypeComputation
from model.discriminator import GradientReversal

from model.utils.losses import BCEFocalLoss
from model.utils.affinity import Affinity
from model.utils.attentions import MultiHeadAttention, HyperGraph
    ...
    def _forward_train(self, images, features, targets=None, score_maps=None):
        ...
        if nodes_2 is not None:  # Both domains have graph nodes
            ...
            if self.with_complete_graph:
                ...
                # arr_s = []
                # arr_t = []
                # labels_s = labels_1.int()
                # labels_t = labels_2.int()
                # for i in range(labels_s.min(),labels_s.max()+1):
                #     l = torch.where(labels_s == i)
                #     if l is not None and l[0].size != 0:
                #         arr_s.append(l[0][0])
                #         if i == 9 :
                #             arr_s.append(l[0][1])
                # for i in range(labels_t.min(),labels_t.max()+1):
                #     l = torch.where(labels_t == i)
                #     if l is not None and len(l[0]) != 0:
                #         arr_t.append(l[0][0])
                #         if i == 9 and len(l[0]) > 1:
                #             arr_t.append(l[0][1])
                # arr_s = torch.tensor(arr_s).to(device=opt.device)
                # arr_t = torch.tensor(arr_t).to(device=opt.device)
                # node_s = torch.index_select(nodes_1, 0, arr_s)
                # node_t = torch.index_select(nodes_2, 0, arr_t)
                # edges_s = edges_1[arr_s][:, arr_s]
                # edges_t = edges_2[arr_t][:, arr_t]
        ...
        for c in labels_exist:
            ...
            if sr_indx.any() and tg_indx.any():  # If the category appear in both domains, we directly collect them!
                ...
                if self.with_semantic_completion:
                    sr_nodes_c = torch.normal(0, 0.01, size=tg_nodes_c.size()).to(tg_nodes_c.device) + sr_nodes_c if len(
                        tg_nodes_c) < 5 \
                        else torch.normal(mean=sr_nodes_c,
                                          std=tg_nodes_c.std(0).unsqueeze(0).expand(sr_nodes_c.size())).to(sr_nodes_c.device)
                else:
                    sr_nodes_c = torch.normal(0, 0.01, size=tg_nodes_c.size()).to(tg_nodes_c.device)
                ...
                if self.with_semantic_completion:
                    tg_nodes_c = torch.normal(0, 0.01, size=tg_nodes_c.size()).to(tg_nodes_c.device) + tg_nodes_c if len(
                        sr_nodes_c) < 5 \
                        else torch.normal(mean=tg_nodes_c,
                                          std=sr_nodes_c.std(0).unsqueeze(0).expand(sr_nodes_c.size())).to(device=opt.device)
                else:
