...
from utils.matrix_show import show_m
...
class V2GConv(torch.nn.Module):
    # Project the sampled visual features to the graph :
...
class GModule(torch.nn.Module):
    ...
    def _forward_aff(self, nodes_1, nodes_2, labels_side1, labels_side2):
        if self.matching_cfg == 'o2o':
            ...
            # M_aff = torch.zeros_like(M)
            result = torch.where(TP_mask == 1, M, torch.tensor(0.0,device=opt.device))


            # show_m(TP_mask.detach().clone(),'TP_mask')
            
            # show_m(result.detach().clone(),'M')
